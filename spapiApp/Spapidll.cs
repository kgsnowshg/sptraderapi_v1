...
public struct SPApiAccInfo
{
    ...
    public double LoanToMR;         //借贷/可按值%
    public double LoanToMV;         //借贷/市值%
    [MarshalAs(UnmanagedType.ByValTStr, SizeConst = 16)]
...
public struct SPApiInstrument
{
    ...
    public byte DecInPrice;    //产品系列的小数位 //玻珇╰计
    public byte InstType;      //产品系列的类型 //玻珇╰摸
};
...
namespace spapiApp
{
    ...
    public delegate void LoadTradeEndReply(string acc_no);
    public delegate void LoadAETradeEndReply();
    public delegate void BusinessDateReply(int business_date);
    ...
    public class Spapidll
    {
        ...
        public static LoadTradeEndReply MyLoadTradeEndReply;
        public static LoadAETradeEndReply MyLoadAETradeEndReply;
        public static BusinessDateReply MyBusinessDateReply;
        ...
        [DllImport(SPAPIDLL, EntryPoint = "SPAPI_LoadProductInfoList", CharSet = CharSet.Ansi)]
        public static extern int SPAPI_LoadProductInfoList();
        ...
        [DllImport(SPAPIDLL, EntryPoint = "SPAPI_RegisterLoadTradeEnd", CharSet = CharSet.Ansi)] 
        public static extern void SPAPI_RegisterLoadTradeEnd(LoadTradeEndReply pLoadTradeEndReply);
        [DllImport(SPAPIDLL, EntryPoint = "SPAPI_RegisterLoadAETradeEnd", CharSet = CharSet.Ansi)]
        public static extern void SPAPI_RegisterLoadAETradeEnd(LoadAETradeEndReply pLoadAETradeEndReply);

        [DllImport(SPAPIDLL, EntryPoint = "SPAPI_SetApiLogPath", CharSet = CharSet.Ansi)]
        public static extern int SPAPI_SetApiLogPath(string path);

        [DllImport(SPAPIDLL, EntryPoint = "SPAPI_SendAccControl", CharSet = CharSet.Ansi)]
        public static extern int SPAPI_SendAccControl(string acc_no, byte ctrl_mask, byte ctrl_level);
        

        [DllImport(SPAPIDLL, EntryPoint = "SPAPI_GetCcyRateCount", CharSet = CharSet.Ansi)]
        public static extern int SPAPI_GetCcyRateCount();
        [DllImport(SPAPIDLL, EntryPoint = "SPAPI_GetCcyRate", CharSet = CharSet.Ansi)]
        public static extern int SPAPI_GetCcyRate(int idx, ref SPApiCcyRate ccy_rate);
        [DllImport(SPAPIDLL, EntryPoint = "SPAPI_GetCcyRateByCcy", CharSet = CharSet.Ansi)]
        public static extern int SPAPI_GetCcyRateByCcy(string ccy, ref double rate);

        [DllImport(SPAPIDLL, EntryPoint = "SPAPI_RegisterBusinessDateReply", CharSet = CharSet.Ansi)]
        public static extern void SPAPI_RegisterBusinessDateReply(BusinessDateReply pBusinessDateReply);
        ...
        public void init()
        {
            ...
            if (InitFlag == 0)                      //是否初始化 0：成功//琌﹍て 0Θ
            {
                ...
                /// </summary>
                ///    
                MyLoadTradeEndReply = new LoadTradeEndReply(LoadTradeEndReplyBack);
                SPAPI_RegisterLoadTradeEnd(MyLoadTradeEndReply);
                MyLoadAETradeEndReply = new LoadAETradeEndReply(LoadAETradeEndReplyBack);
                SPAPI_RegisterLoadAETradeEnd(MyLoadAETradeEndReply);
                ...
                MyBusinessDateReply = new BusinessDateReply(BusinessDateReplyBack);
                SPAPI_RegisterBusinessDateReply(MyBusinessDateReply);
            }
        }
        ...
        public void R_SPAPI_GetAccInfo()
        {
            ...
            if (rc == 0)
            {
                if (DllShowTextData != null)
                {
                    ...
                    DllShowTextData(" CashBal:" + acc_info.CashBal.ToString() + " MarginCall:" + acc_info.MarginCall.ToString() + " CommodityPL:" + acc_info.CommodityPL.ToString() + " LockupAmt:" + acc_info.LockupAmt.ToString() + ", Loan2MR:"+ acc_info.LoanToMR.ToString() + ", Loan2MV:" + acc_info.LoanToMV.ToString());
                }
            }
        }
        ...
        public void R_SPAPI_AddOrder(char buy_sell, string pirce, string prodCode, string qty, string clorderid, string decinprice, bool is_ao)
        {
            int rc;
            ...
            {
                order.OrderType = Spcommon.ORD_LIMIT;
        ...
        public static int AddOrder(char buy_sell, string pirce, string prodCode, string qty, string clorderid, string decinprice)
        {
            int rc;
            SPApiOrder order = new SPApiOrder();
            string acc;
        ...
        public static int DeleteOrder(string order_id, string prod_code)//modif xiaolin 2012-12-27
        {
            int rc, int_order_no;
            SPApiOrder order = new SPApiOrder();
            string acc;
        ...
        {
            SPApiOrder order = new SPApiOrder();
            int count = SPAPI_GetOrderCount();  //订单数量
            return count;
        }
        public static string[] GetOrder()
        {
            ...
            for (i = 0; i < count; i++)
            {
                ...
                {
                    order_info = "[Acc No:" + order.AccNo + "] Order#=" + order.IntOrderNo.ToString() + " Status:" + order.Status.ToString() + " B/S=" + Convert.ToChar(order.BuySell) + " ProdCode=" + order.ProdCode + " Price=" + order.Price.ToString() + " Qty=" + order.Qty.ToString() + ",ClOrderId=" + order.ClOrderId;
                    arr[0] = order.IntOrderNo.ToString();
                    arr[1] = order.ProdCode;
                    arr[2] = (Convert.ToChar(order.BuySell)).ToString();
                    arr[3] = order.Price.ToString();
                    arr[4] = order.Qty.ToString();
                    return arr;
                }
            }
        ...
        public static int GetLastOrder()
        {
            ...
            if (count == 0)
            {
                string update = File.ReadAllText("real/waitlist.json", Encoding.Default);
                ...
                File.WriteAllText("real/waitlist.json", updatestring);//将内容写进jon文件中
            }
            rc = SPAPI_GetOrder(i, ref order); //从第一个开始取
            if (rc == 0)
            {
                int qty= Convert.ToInt32(order.Qty);
                if ((Convert.ToChar(order.BuySell)).ToString()== "S")
                {
                    qty = qty * -1;
                }
                ...
                string update = File.ReadAllText("real/waitlist.json", Encoding.Default);
                JObject updates = JObject.Parse(update);
                updates["user"][0]["id"] = order.IntOrderNo.ToString();
                updates["user"][0]["BS"] = (Convert.ToChar(order.BuySell)).ToString();
                updates["user"][0]["position"] = qty;
                updates["user"][0]["price"] = order.Price.ToString();
                updates["user"][0]["cases"] = order.ClOrderId.ToString();
                string updatestring = Convert.ToString(updates);//将json装换为string
                File.WriteAllText("real/waitlist.json", updatestring);//将内容写进jon文件中
        ...
        public static int ChangeOrder(string intOrderNo, string chg_price, string chg_clorderid)
        {
            int rc;
            SPApiOrder order = new SPApiOrder();
            string acc;
        ...
        public void ApiOrderRequestFailedBack(byte action, ref SPApiOrder order, int err_code, string err_msg) //modif xiaolin 2012-12-27
        {
            if (DllShowTextData != null)
            {
                DllShowTextData("Order Request Failed:Order#:" + order.IntOrderNo.ToString() + " [" + err_code.ToString() + " (" + err_msg + ")], Action=" + action.ToString());
            }
        }
        ...
        public static int GetPosByProduct(string prod_code)
        {
            int rc;
            SPApiPos pos = new SPApiPos();
            int qty=0;
        ...
        { 
            ...
            for (i = 0; i < count; i++)
            {
                ...
                if (rc == 0)
                {
                    trade_info = "("+trade.AccNo+")Order Id:" + trade.IntOrderNo.ToString() + " Trade Id:" + trade.TradeNo.ToString() + " Product:" + trade.ProdCode + " B/S:" + Convert.ToChar(trade.BuySell) + " Trade Count:" + trade.Qty.ToString() + " Trade Price:" + trade.Price.ToString() + " TradeTime:" + unixTimeToStandTime(trade.TradeTime, 3);
        ...
        public static string[] LastGetTrade()
        {
            int i, rc, count;
            SPApiTrade trade = new SPApiTrade();
            ...
            {
                ary[0] = "1";
                ary[1] = trade.BuySell.ToString();
                ary[2] = trade.Qty.ToString();
                ary[3] = trade.Price.ToString();
                ary[4] = trade.ClOrderId.ToString();
                return ary;
            }
        ...
        {
            string trade_info;
            trade_info = "Trade Report(" + trade.Initiator + ":" + trade.AccNo + ":" + trade.BuySell + "):" + "Prod:" + trade.ProdCode + ",Req No:" + rec_no.ToString() + ",Status:" + trade.Status.ToString() + ",Order#" + trade.IntOrderNo.ToString() + ",trade_no=" + trade.TradeNo.ToString() + ",trade_price=" + trade.Price.ToString() + ",Avg_price=" + trade.AvgPrice.ToString() + ",trade_qty=" + trade.Qty.ToString() + ",Ref:" + trade.Ref;
            int position = GetPosByProduct(trade.ProdCode);
            realposition.updateposition(position);//更新持倉
            realposition.updaterecord(position);
            ...
            GetLastOrder();
            StreamReader sr = new StreamReader(@"real/log2.txt");
            string result = sr.ReadToEnd();
            sr.Close();
            StreamWriter sw = new StreamWriter(@"real/log2.txt");
            sw.WriteLine(result);
            sw.WriteLine("price: " + trade.Price.ToString());
            sw.WriteLine("B/S:" + trade.BuySell.ToString());
            sw.WriteLine("CASE: " + trade.ClOrderId.ToString());
            sw.WriteLine("time: " + trade.TradeTime.ToString());
            sw.Close();
            string update = File.ReadAllText("real/recordlasttrade.json", Encoding.Default);
            JObject updates = JObject.Parse(update);
            updates["user"][0]["BS"] = trade.BuySell.ToString();
            updates["user"][0]["price"] = trade.Price.ToString();
            updates["user"][0]["cases"] = trade.ClOrderId.ToString();
            updates["user"][0]["time"] = nowsec.ToString();
            string updatestring = Convert.ToString(updates);//将json装换为string
        ...
        {
            string prc_info;
            prc_info = "Order Report(" + order.Initiator+ ":" + order.AccNo + ":" + Convert.ToChar(order.BuySell) + "):ReqNo:" + rec_no.ToString() + ",Status:" + order.Status.ToString() + ",Order#" + order.IntOrderNo.ToString() + ",ProdCode=" + order.ProdCode + ",Price=" + CheckAO(order.Price) + ",Qty=" + order.Qty.ToString() + ",TradeQty=" + order.TradedQty.ToString() + ",TotalQty=" + order.TotalQty.ToString() + ",ClOrderId=" + order.ClOrderId + ",TimeStamp:" + unixTimeToStandTime(order.TimeStamp, 3)+",Ref:" + order.Ref;
            int i, rc, count;
            count = SPAPI_GetOrderCount();  //订单数量
            i = count - 1;
            if (count == 0)
        ...
        private string CheckAO(double price)
        {
            int i;
            double n_price;
            i = 1;
            n_price = price;
            while (Spcommon.AO_PRC != n_price)
            {
                n_price *= 10;
                if (i == 6) break;
                else i++;
            }
            if (n_price == Spcommon.AO_PRC) return "AO";
        ...
        public void ApiPriceUpdateBack(ref SPApiPrice prc)
        {
            string prc_info;
            //獲得細期code
            string mhi = prc.ProdCode;
             mhi = mhi.Replace("HSI", "MHI");
            //讀取恆生指數
            string result = url.geturlresult();
            //讀取時間
            int hsi = Convert.ToInt32(result);
            int hour = time.hour();
            int min = time.min();
            int sec = time.sec();
            int totalsec = Convert.ToInt32(hour) * 60 * 60 + Convert.ToInt32(min) * 60 + Convert.ToInt32(sec);
            //獲得持倉數及買入點數及OREDER表~(實戰時用)
            LoadOrderReport();
            LoadTradeReport();
            int position = GetPosByProduct(mhi);
            string price = string.Join("|", LastGetTrade());
            string order = string.Join("|", GetOrder());
            realposition.updateposition(position);//更新持倉數量
            realposition.updaterecord(position);//更新持倉資料
            GetLastOrder();// 更新waitlist
            int countresult = countorder();
            realposition.delmorewait(countresult, mhi); //del過多waitlist
            string realwaitlist = string.Join("|", realuser.getwaitlist());
            realuser.saveuser(prc.Last[0], realwaitlist, hour, min, sec);
            realpositions.update(prc.Last[0],mhi); //止賺

            //手動部分
            setting.buy(countresult, mhi);//手動買
            setting.sell(countresult, mhi);//手動賣
            setting.delwait(countresult, mhi);//刪除waitlist
            int programopen = setting.program();//program開關 0為關 1為升

            //檢查有無order,並寫入紀錄 (專為測試用)
            string waitlist = string.Join("|", user.getwaitlist());
            user.saveuser(prc.Last[0], waitlist, hour, min, sec);
            positions.update(prc.Last[0]); //止賺
            if (totalsec > 32700)
            {
                //寫入每分鐘/秒恆生指數
                hsia.min(hour, min, sec, hsi);
                //寫入每分鐘/秒期指
                hsis.min(hour, min, sec, prc.Open, prc.Last[0], prc.TurnoverVol, prc.Bid[0], prc.Bid[1], prc.Bid[2], prc.Bid[3], prc.Bid[4], prc.BidQty[0], prc.BidQty[1], prc.BidQty[2], prc.BidQty[3], prc.BidQty[4], prc.Ask[0], prc.Ask[1], prc.Ask[2], prc.Ask[3], prc.Ask[4], prc.AskQty[0], prc.AskQty[1], prc.AskQty[2], prc.AskQty[3], prc.AskQty[4]);

                //分析數據
                string result1 = string.Join("|", signals.buy(hour, min, sec, prc.Last[0])); //持續50數量成交 買入
                string result2 = string.Join("|", signals.sell(hour, min, sec, prc.Last[0])); //持續50數量成交 賣出
                string result3 = string.Join("|", signals.sec(hour, min, sec, prc.Last[0]));  //突然升/趺10 
                string result4 = string.Join("|", signalm.hsi(hour, min, sec, prc.Last[0]));  //3分鐘分析 1分鐘平倉 讀取期指
                string result5 = string.Join("|", signalm.hsia(hour, min, sec, hsi));         //讀取恆指
                string result6 = string.Join("|", waitsignal.controlwaitsignal(prc.Last[0], mhi)); //平倉快買signal

                operate.deal(result1, result2, result3, result4, result5, result6, mhi);

                //執行平倉/買賣操作
                if (programopen == 1)
                {
                    if (totalsec < 41400 || (totalsec > 46500 && totalsec < 55800) || position != 0)
                    {
                        realoperate.deal(result1, result2, result3, result4, result5, result6, mhi);
                        trade.tradeoperate();
                    }
                }
            }
            prc_info = "API PriceUpdate: HSI=" + hsi + ",ProdCode=" + prc.ProdCode + ",Bid1=" + prc.Bid[4] + ",Bid1=" + prc.Bid[3] + ",Bid1=" + prc.Bid[2] + ",Bid1=" + prc.Bid[1] + ", Bid=" + prc.Bid[0] + ", BQty=" + prc.BidQty[0] + ", Ask=" + prc.Ask[0] + ", AQty=" + prc.AskQty[0] + " LastTime:" + prc.LastTime[0].ToString() + " DecInPrice:" + prc.DecInPrice.ToString();
            if (DllShowTextData != null) DllShowTextData(prc_info);
        }
        ...
        public void LoadTradeEndReplyBack(string acc_no)
        {
            if (DllShowTextData != null) DllShowTextData("Load Trade End: [Acc No: "+ acc_no +"]");      
        }

        public void LoadAETradeEndReplyBack()
        {
            if (DllShowTextData != null) DllShowTextData("Load AE Trade End");
        }
        ...
        public void R_SPAPI_GetProductAndCount()
        {
            ...
            for (i = 0; i < count; i++)
            {
                ...
                if (rc == 0)
                {
                    ...
                    prod_info = "Instrument:" + prod.InstCode + ",Product Code:" + prod.ProdCode + ",Product Name:" + prod.ProdName2 + ",Product Date:" + expdate + " TickSize:"+ prod.TickSize.ToString();
        ...
        public void TickerCallBack(ref SPApiTicker ticker)
        {
            ...
            ticker_info = "Ticker Code:" + ticker.ProdCode + " Trade Price:" + ticker.Price.ToString() + " Count:" + ticker.Qty.ToString() + " Time:" + unixTimeToStandTime(ticker.TickerTime, 3) + " " + ticker.TickerTime.ToString();
        ...
        public void PServerLinkStatusUpdateBack(short host_id, int con_status)
        {
            ...
            if (DllShowTextData != null) DllShowTextData("User Connection Status: " + host_id.ToString()+", "+host_name+", "+msg + "["+con_status+"]");
        }
        ...
        public void R_SPAPI_LoadProductInfoListByCode(string product)
        {
            ...
            /*
            int rc;
            string[] product1 = {"GC","CL","HSI"};

            for (int i = 0; i < product1.Count(); i++)
            {
                product = product1[i];
                rc = SPAPI_LoadProductInfoListByCode(product);
                DllShowTextData("Load...:" + rc.ToString() + "P:" + product1[i]);
            }*/
        }
        ...
        public void BusinessDateReplyBack(int business_date)
        {
            Spcommon.Business_Date = business_date;
        }
        ...
        public void R_SPAPI_SetApiLogPath(string path)
        {
            int rc;

            rc = SPAPI_SetApiLogPath(path);
            if (DllShowTextData != null) DllShowTextData("Set Api Log Path: rc =" + rc.ToString());
        }

        public void R_SPAPI_SendAccControl(string acc, byte ctrl_mask, byte ctrl_level)
        {
            int rc;

            rc = SPAPI_SendAccControl(acc, ctrl_mask, ctrl_level);
            if (DllShowTextData != null) DllShowTextData("Send Acc Control Report Reply:: rc =" + rc.ToString());
        }

        public void R_SPAPI_GetCcyRateCount()
        {
            int count;
            string ccy_info;

            count = SPAPI_GetCcyRateCount();
            ccy_info = "User Id:" + Spcommon.acc_no + "  Ccy Rate Count:" + count.ToString();
            if (DllShowTextData != null) DllShowTextData(ccy_info);

        }

        public void R_SPAPI_GetCcyRate()
        {
            int i, rc, count;
            string ccy_info;

            SPApiCcyRate ccy_rate = new SPApiCcyRate();

            count = SPAPI_GetCcyRateCount(); 
            if (count == 0) if (DllShowTextData != null) DllShowTextData("No Ccy Rate!");
            for (i = 0; i < count; i++)
            {
                rc = SPAPI_GetCcyRate(i, ref ccy_rate);  
                if (rc == 0)
                {
                    ccy_info = "Ccy:" + ccy_rate.Ccy + " Rate:" + ccy_rate.Rate.ToString();
                    if (DllShowTextData != null) DllShowTextData("[Ccy Rate: " + (i + 1).ToString() + "] " + ccy_info);
                    ccy_info =  "";
                }
                else
                    if (DllShowTextData != null) DllShowTextData("[Ccy Rate: " + (i + 1).ToString() + "] " + "Read Failure!");
            }
        }

        public void R_SPAPI_GetCcyRateByCcy(string ccy_code)
        {
            int rc;
            string ccy_info;
            double rate = 0;

            rc = SPAPI_GetCcyRateByCcy(ccy_code, ref rate);
            if (rc == 0)
            {

                ccy_info = "Ccy:" + ccy_code + " Rate:" + rate.ToString(); 
                if (DllShowTextData != null) DllShowTextData(ccy_info);
                ccy_info = "";
            }
            else
                if (DllShowTextData != null) DllShowTextData("No:" + ccy_code + " Ccy Rate Info!");
        }

        internal void SPAPI_GetPriceByCode(string v1, ref double v2)
        {
