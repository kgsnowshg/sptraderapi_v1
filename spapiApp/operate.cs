...
namespace spapiApp
{
    class operate
    {
        ...
        {
            //讀取waitlist
            string getwaitlist = string.Join("|", user.getwaitlist());
            string[] waitlist = getwaitlist.Split('|');
            int wprice = Convert.ToInt32(waitlist[0]);
            int wposition = Convert.ToInt32(waitlist[1]);
            string wcases = waitlist[2].ToString();
            string wtime = waitlist[3].ToString();
            ...
            for (int i=0;i<5;i++)
            {
                if ((Convert.ToInt32(ary[i,0])>0) && (Convert.ToInt32(ary[i, 0])>action))
                {
                    ...
                    if (action == 2 && uuposition!=0)
                    {
                        ...
                        {
                            user.savewaitlist(price, 1, cases);
                            //加入waitlist
                        }
            ...
            if (signal > 0)
            {
                ...
                if (uuposition == 0)
                {
                    if (bs == 1 && wposition == 0)
                    {
                        user.savewaitlist(price, bs, cases);
                        //加入waitlist
                    }
                    if (bs == 1 && wposition == -1)
                    {
